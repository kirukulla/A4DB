 ...
 * @since 08/20/2017
 *
 * This class is ancestor to all Fact classes.
 */
public abstract class DataWriter implements Runnable {
	...
	private int _status = DataWriter.INACTIVE;
	 ...
	 * @throws Exception
	 */
	public DataWriter(String dbName, String datasetName, int operationMode) throws Exception {
	 ...
	 * @throws Exception
	 */
	public DataWriter(String dbName, String datasetName, int recordCount, int encoding, int dataLength, short decimals, int segmentNo) throws Exception {
	...
	private void setFileType() {
		this._fileType = CheckSum.FILE_TYPE_FACT;
	}
	...
	*/
	...
	private void checkEncodingNew(int encoding, int recordCount, int dataLength, short decimals, int segmentNo) throws Exception {
		...
		// check File Type
		if (this._fileType == CheckSum.FILE_TYPE_FACT) {
			if (encoding < CheckSum.FACT_ENCODE_TYPE_BYTE || encoding > CheckSum.FACT_ENCODE_TYPE_ALPHAN)
				...
				if (encoding == CheckSum.FACT_ENCODE_TYPE_BYTE)
					...
					if (dataLength <= 0 || dataLength > HASIDSConstants.FACT_MAX_ALPHAN_LENGTH)
						throw new Exception ("Max data length allowed in bytes is " + HASIDSConstants.FACT_MAX_ALPHAN_LENGTH + ", Atomize to this length");
					// we will reserve one byte for storing the actual length
					this._dataLength = dataLength + 1;
				}
				else if (encoding == CheckSum.FACT_ENCODE_TYPE_DECIMAL) { // stored as aplha numeric decimals rounded
					if (decimals > dataLength)
		...
		int checkLength = this._dataLength;
		if (encoding == CheckSum.FACT_ENCODE_TYPE_ALPHAN)
			checkLength = checkLength - 1;
		
		System.out.println("Encoding: " + this._encoding + ", Check Length : " + checkLength);
		
		if (recordCount <= 0 || recordCount > HASIDSConstants.DIM_MAX_RECORDS/checkLength) // 2 billion bytes
			throw new Exception ("Record count : " + recordCount + " must be > 0 and <= " + (HASIDSConstants.DIM_MAX_RECORDS/checkLength));
	...
	private void createSegment() throws Exception {
		this._status = DataWriter.ACTIVE;
		//byte buf = 0;
		...
		try {
			...
			this._status = DataWriter.FAILED;
			throw new Exception(e.getMessage());
		}
		finally {
			if (this._status == DataWriter.ACTIVE)
				this._status = DataWriter.COMPLETE;
	...
	private Hashtable<Integer, Byte> setWriteDataPositionBufferOnline(int[] position, byte[] values, boolean allowPartial, boolean retryFlag) throws Exception {
		...
		synchronized (this) {
			...
			this._positionTable = DataWriter.LOCK_TABLE.get(this._dbName + "|" + this._datasetName);
			if (this._positionTable == null) {
				this._positionTable = new Hashtable<Integer, Object>(position.length);
				DataWriter.LOCK_TABLE.put(this._dbName + "|" + this._datasetName, this._positionTable);
			}
		}
	...
	protected static Hashtable<Integer, Byte> getLockedKeysByte(String dbName, String datasetName) {
		...
		Hashtable<Integer, Object> positionTable = DataWriter.LOCK_TABLE.get(dbName + "|" + datasetName);
